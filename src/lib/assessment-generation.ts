...
export async function generateAssessment(input: { skill: string; difficulty: Difficulty; count: number; previousQuestionHashes?: string[] }): Promise<GeneratedAssessment> {
  ...
  if (!apiKey?.trim()) return fallbackAssessment(input, "GEMINI_API_KEY is not configured.");

  try {
    const ai = new GoogleGenAI({ apiKey });
    const response = await ai.models.generateContent({
      model: "gemini-2.5-flash",
      contents: `Generation request ${randomUUID()}. Generate ${input.count} unique ${input.difficulty} MCQs for ${input.skill}. Return JSON only with a questions array. Every question needs exactly four options with ids A-D, one correctOption, explanation, topic, skill, difficulty. Avoid these prior question hashes: ${(input.previousQuestionHashes ?? []).join(", ")}`,
    ...
    console.error("Gemini assessment generation failed:", reason);
    return fallbackAssessment(input, "Gemini generation failed. PRAMAAN switched to its verified assessment bank.");
  }
}

function fallbackAssessment(
  input: { count: number; difficulty: Difficulty },
  reason: string
): GeneratedAssessment {
  return {
    questions: fallbackQuestionSet(input.count, input.difficulty),
    codingProblem: FALLBACK_CODING,
